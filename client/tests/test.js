...
require(['cursor'], function(cursor) {
	...
	Sandbox(function(cursor) {
		cursor.row = 1;
		cursor.shift('up');
		equal(cursor.row, 0, 'cursor.shift(\'up\') moves the column up by one')

		cursor.shift('up');
		equal(cursor.row, 0, 'cursor.shift(\'up\') does not move up if the cursor is on the top line')
	});

	Sandbox(function(cursor) {
		cursor.shift('down');
		equal(cursor.row, 1, 'cursor.shift(\'up\') moves the column up by one')
